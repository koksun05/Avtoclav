...
float P = 0.0;                   // тиск
int K_tempr_x = 385;               // коєфіціент для термодатчика
bool tenOn = false;
bool U_low = false;              
bool tenErr = false;
bool  P_err = false;
...
const int ADDR_Ktemp = 12;     // int (2 байти)
const int ADDR_hTen  = 14;     // float
const int ADDR_hTim  = 18;     // float
...
float volt = raw * 0.0000625;
  ...
  float K_tempr = K_tempr_x / 100000.0f;
  float temp = (Rpt - 50.0) / (50.0 * K_tempr);
  return temp;
}
...
EEPROM.get(ADDR_Ktemp, K_tempr_x);
if (K_tempr_x < 300 || K_tempr_x > 450) K_tempr_x = 385;

EEPROM.get(ADDR_hTen, histerTen);
if (histerTen < 0.1 || histerTen > 5.0) histerTen = 0.2;

EEPROM.get(ADDR_hTim, histerTimer);
if (histerTimer < 0.1 || histerTimer > 5.0) histerTimer = 0.5;
}
...
void saveSettingsToEEPROM() {
  ...
  EEPROM.put(ADDR_Ktemp, K_tempr_x);
  EEPROM.put(ADDR_hTen, histerTen);
  EEPROM.put(ADDR_hTim, histerTimer);
  EEPROM.commit();
}
//---------------------------------------------------------------------
void buttons() {
  ...
  if (menuBtn == HIGH && menuLast == LOW) {
    if (!menuLongFired) {
      // short press action
      if (menu == 0){
        if(P > 0.9 && P < 2.8){work_flag = !work_flag;}
...
if (menu == 1) {
  switch(editField) {
    ...
    case 0: h = (h + 1) % 4; break;
    case 1: m = (m + 1) % 60; break;

    case 4:
      K_tempr_x++;
      if (K_tempr_x > 450) K_tempr_x = 450;
    break;

    case 2:
      histerTen += 0.1;
      if (histerTen > 5.0) histerTen = 5.0;
    break;

    case 3:
      histerTimer += 0.1;
      if (histerTimer > 5.0) histerTimer = 5.0;
    break;
  }
  menu2_lastActivity = now;
} else {
...
if (menu == 1) {
  switch(editField) {
    ...
    case 0: h = (h + 3) % 4; break;
    case 1: m = (m + 59) % 60; break;

    case 4:
      K_tempr_x--;
      if (K_tempr_x < 300) K_tempr_x = 300;
    break;

    case 2:
      histerTen -= 0.1;
      if (histerTen < 0.1) histerTen = 0.1;
    break;

    case 3:
      histerTimer -= 0.1;
      if (histerTimer < 0.1) histerTimer = 0.1;
    break;
  }
  menu2_lastActivity = now;
}
...
void readADS_andCalc() {
  ...
  if (fabs(tenCurrent) < 0.05f) tenCurrent = 0.0f;
...
void display_2(){
   ...
   // ---- histerTen ----
   lcd.setCursor(0,1);
   if (editField == 3) lcd.print(">");
   else lcd.print(" ");
   lcd.print("hTen=");
   lcd.print(histerTen,1);
   // ---- histerTimer ----
   lcd.setCursor(0,2);
   if (editField == 4) lcd.print(">");
   else lcd.print(" ");
   lcd.print("hTim=");
   lcd.print(histerTimer,1);
      // ---- K_tempr ----
   lcd.setCursor(0,3);
   if (editField == 4) lcd.print(">");
   else lcd.print(" ");
   lcd.print("K=");
   lcd.print(K_tempr_x / 100000.0f, 5);
}
...
if(work_flag){
 if (P < 0.9 || P > 4.0){work_flag = false; P_err = true; }
  if(t_fakt >= setTemp - histerTen){
    timer_flag = true; tenOn = false;
    digitalWrite(RELAY_PIN,LOW);
  } else if(t_fakt <setTemp - histerTen) {tenOn=true;                    // гістерезис для температури 
    lcd.setCursor(17, 1); lcd.print("TEN");
    digitalWrite(RELAY_PIN,HIGH);
  }
   if(t_fakt <setTemp - histerTimer){timer_flag = false;} // гістерезис для зворотнього відліку
